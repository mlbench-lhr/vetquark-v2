  ...
  try {
    ...
    if (method === "pix") {
      ...
      customer: {
        ...
        type: "individual",
        document_type: "CPF",
        document: "11824363000".replace(/\D/g, ""),
        // phones: {
      ...
      const errJson: any = await r.json().catch(() => null);
      if (!r.ok || !errJson || !errJson.id) {
        console.error("PagarmeCreateV5 failed", JSON.stringify({ status: r.status, body: errJson }));
        return NextResponse.json(
          { error: "providerError", message: errJson.message || "Failed to create payment", details: errJson },
          { status: 500 }
        );
      }
      
      const order = errJson as PagarmeOrder;
      console.log("PagarmeCreateV5 success", JSON.stringify({ orderId: order.id, status: order.status }));
      
      const charge = order.charges?.[0];
      const pixQrCode = charge?.payment?.pix?.qr_code || "";
      const pixQrCodeBase64 = charge?.payment?.pix?.qr_code_base64 || "";
      
      return NextResponse.json({ ok: true, method: "pix", pixQrCode, pixQrCodeBase64 });
    }
    
    return NextResponse.json({ ok: true });
  } catch (err: any) {
    console.error("PaymentCreate exception", err);
