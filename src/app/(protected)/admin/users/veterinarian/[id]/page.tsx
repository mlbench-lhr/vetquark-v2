...
export default function VeterinarianUserDetailPage() {
  ...
  const displayName = user?.tradeName?.trim() ? user.tradeName : user?.fullName || "";
  ...
  return (
    <BasicStructureWithName
      ...
      subHeading="View veterinarian profile information."
      showBackOption
    >
      ...
      <BoxProviderWithName name="Professional" className="border-gray-200">
        <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="rounded-xl bg-[#F5F6F6] px-4 py-3">
            <div className="text-xs text-gray-500">CRMV</div>
            <div className="text-sm font-medium text-gray-900">{user?.crmv?.trim() || (loading ? "—" : "—")}</div>
          </div>
          <div className="rounded-xl bg-[#F5F6F6] px-4 py-3">
            <div className="text-xs text-gray-500">CRMV State</div>
            <div className="text-sm font-medium text-gray-900">{user?.crmvState?.trim() || (loading ? "—" : "—")}</div>
          </div>
          <div className="rounded-xl bg-[#F5F6F6] px-4 py-3">
            <div className="text-xs text-gray-500">MAPA Registration</div>
            <div className="text-sm font-medium text-gray-900">{user?.mapaRegistration?.trim() || "—"}</div>
          </div>
          <div className="rounded-xl bg-[#F5F6F6] px-4 py-3">
            <div className="text-xs text-gray-500">Expertise</div>
            <div className="text-sm font-medium text-gray-900">
              {user?.expertise?.length ? user.expertise.join(", ") : "—"}
            </div>
          </div>
        </div>
      </BoxProviderWithName>
