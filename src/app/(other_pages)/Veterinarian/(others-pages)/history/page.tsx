...
function ActionButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      disabled={!!disabled}
      className={`inline-flex h-7 w-7 items-center justify-center rounded-full ${className} ${disabled ? "cursor-not-allowed opacity-50" : ""}`}
...
function ReportCard({ item, onDownload, onShare }: { item: ReportHistoryItem; onDownload: () => void; onShare: () => void }) {
  const isPaymentPending = item.paymentStatus === "pending" && !!item.paymentLinkId && !!item.patientId;
  const viewHref = useMemo(() => {
    if (!isPaymentPending) return `/Veterinarian/history/detail/${item.id}`;
    const params = new URLSearchParams();
    params.set("patientId", item.patientId);
    params.set("paymentLinkId", item.paymentLinkId);
    params.set("step", "timer");
    return `/Veterinarian/new-reading?${params.toString()}`;
  }, [isPaymentPending, item.id, item.patientId, item.paymentLinkId]);
    ...
    <div className="rounded-[12px] bg-white px-4 py-3 shadow-theme-xs">
      ...
      <div className="mt-2.5 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <ActionButton variant="success" label="Share report" onClick={onShare} disabled={isPaymentPending}>
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!mounted) return;
        if (!res.ok) {
          const msg = typeof (data as any)?.error === "string" ? (data as any).error : "Failed to load reports";
          throw new Error(msg);
        }
      ...
      <div className="mx-auto w-full h-full pb-6 pt-[calc(env(safe-area-inset-top)+20px)]">
        ...
        <div className="mt-6 space-y-4 p-4 h-full rounded-[16px] bg-[#F5F6F6]">
          {loading ? (
            ...
            <div className="text-[14px] text-gray-500">No reports yet.</div>
          ) : (
