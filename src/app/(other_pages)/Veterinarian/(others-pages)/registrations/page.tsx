...
import { PawPrint, User, ChevronRight, ChevronDown, Plus, History, Pencil, X, Search, Phone, Mail, MessageCircle } from "lucide-react";
...
function RegistrationsListContent() {
  ...
  return (
    <div className="min-h-[100dvh] w-full bg-white pb-28">
      <div className="mx-auto w-full max-w-md">

        {/* Header */}
        <div className="flex items-center justify-between px-4 pt-4 pb-3">
          <h1 className="text-[22px] font-bold text-[#3F78D8]">{lang === "pt" ? "Cadastros" : t("registrations.title")}</h1>
          <div className="flex items-center gap-3">
            <button
              ...
              aria-label="Search"
            >
              <Search className="h-5 w-5 text-[#9AA4AF]" />
            </button>
            <button
              onClick={() => router.push("/Veterinarian/notifications")}
              className="flex h-9 w-9 items-center justify-center rounded-full bg-[#3F78D8]"
        ...
        <div className="flex items-center gap-2 px-4 pb-2 overflow-x-auto">
          <button
            ...
            className={`flex items-center gap-1.5 rounded-full border px-4 py-2 text-[13px] font-medium whitespace-nowrap transition-colors ${activeTab === "patients"
              ? "border-[#3F78D8] text-[#3F78D8] bg-white"
              : "border-transparent text-[#6B7280] bg-[#F5F6F6]"
              }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 rounded-full border px-4 py-2 text-[13px] font-medium whitespace-nowrap transition-colors ${activeTab === "guardians"
              ? "border-[#3F78D8] text-[#3F78D8] bg-white"
              : "border-transparent text-[#6B7280] bg-[#F5F6F6]"
              }`}
          >
          ...
          {activeTab === "patients" ? (
            <>
              <button
                ...
                className="flex items-center gap-1.5 rounded-full bg-[#F5F6F6] px-3 py-2 text-[13px] font-medium text-[#6B7280] whitespace-nowrap"
              >
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="text-[15px] font-semibold text-[#111827]">{patient.name}</h3>
                    ...
                    <div className="mt-3 flex items-center gap-2">
                      ...
                      <span className="rounded-full border border-gray-200 px-2.5 py-0.5 text-[11px] font-medium text-[#6B7280]">
                    ...
                    <div className="mt-3 flex gap-2">
                      <Button
                        ...
                        className="flex-1 rounded-full bg-[#F5F6F6] h-9 px-3 text-[12px] font-normal text-[#374151] hover:bg-gray-100 gap-1.5"
                      >
                        <svg width="16" height="8" viewBox="0 0 17 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <rect width="17" height="8" rx="1" fill="#374151" />
                          <rect x="1" y="1" width="5" height="6" rx="1" fill="white" />
                          <rect x="7" y="1" width="5" height="6" rx="1" fill="white" />
                        </svg>
                        {lang === "pt" ? "Iniciar Urinólise" : t("registrations.startUrinalysis")}
                      </Button>
                      <Button
                        variant="ghost"
                        size="sm"
                      ...
                      <div className="mt-3 border-t border-gray-100 pt-3 space-y-2">
                        {patient.microchip ? (
                          <div className="flex items-center gap-2">
                            <span className="text-[12px] font-semibold text-[#374151]">{lang === "pt" ? "Microchip:" : "Microchip:"}</span>
                            <span className="text-[12px] text-[#6B7280]">{patient.microchip}</span>
                          </div>
                        ) : null}
                        {patient.allergies ? (
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="text-[12px] font-semibold text-[#374151]">{lang === "pt" ? "Alergias:" : "Allergies:"}</span>
                            {patient.allergies.split(",").map((allergy) => (
                              <span key={allergy.trim()} className="rounded-md bg-red-100 px-2 py-0.5 text-[11px] font-medium text-red-600">
                                {allergy.trim()}
                              </span>
                            ))}
                          </div>
                        ...
                        {patient.planName ? (
                          <div className="flex items-center gap-2">
                            <span className="text-[12px] font-semibold text-[#374151]">{lang === "pt" ? "Plano de Saúde:" : "Health Plan:"}</span>
                            <span className="text-[12px] text-[#6B7280]">{patient.planName}</span>
                          </div>
                        ) : null}
                      </div>
                    ) : null}
            ...
            guardiansLoading ? (
              ...
              guardians.map((guardian) => (
                ...
                >
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      <h3 className="text-[16px] font-bold text-[#111827]">{guardian.name}</h3>
                      <div className="mt-1 flex items-center gap-2">
                        <span className="text-[13px] text-[#6B7280]">CPF:</span>
                        {guardian.idNumber ? (
                          <span className="text-[13px] text-[#374151]">{guardian.idNumber}</span>
                        ) : (
                      ...
                      {guardian.pets && guardian.pets.length > 0 ? (
                        <div className="mt-2 flex items-center gap-2 flex-wrap">
                          <span className="text-[12px] font-semibold text-[#374151]">{lang === "pt" ? "Pets:" : "Pets:"}</span>
                          {guardian.pets.map((pet) => (
                            <span key={pet} className="rounded-full border border-gray-200 px-2.5 py-0.5 text-[11px] font-medium text-[#6B7280]">
                              {pet}
                            </span>
                          ))}
                        </div>
                    ...
                    <div className="flex items-center gap-1.5">
                      {guardian.phone ? (
                        <a
                          ...
                          className="flex h-8 w-8 items-center justify-center rounded-full bg-[#3F78D8]"
                        >
                      ...
                      <a
                        ...
                        onClick={(e) => {
                          e.preventDefault();
                          router.push(`/Veterinarian/home/guardianDetails/${encodeURIComponent(guardian.id)}`);
                        }}
                        className="flex h-8 w-8 items-center justify-center rounded-full bg-[#3F78D8]"
                      >
        ...
        {/* Bottom Add Button */}
        <div className="fixed bottom-[calc(env(safe-area-inset-bottom)+88px)] left-4 right-4 z-10">
          <Button
            onClick={() => {
              if (activeTab === "patients") {
                router.push("/Veterinarian/patient");
              } else {
                router.push("/Veterinarian/patient/new_guardian");
              }
            }}
            className="h-12 w-full rounded-xl bg-[#3F78D8] text-[15px] font-medium text-white hover:bg-[#3F78D8]/90 shadow-md gap-2"
          >
            <Plus className="h-5 w-5" />
            {activeTab === "patients"
              ? (lang === "pt" ? "Adicionar Paciente" : t("registrations.addNewPatientButton"))
              : (lang === "pt" ? "Adicionar Tutor" : t("registrations.addGuardianButton"))}
          </Button>
        </div>
      </div>
